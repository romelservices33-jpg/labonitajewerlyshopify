...
function updateStorefrontFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');

  // 1. BEST SELLERS VITRINE REPLACEMENT
  // Search for: <section class="editorial-vitrine-section" id="mas-vendidos" ... <div class="editorial-vitrine-grid"> ... </div> \n </section>
  const bestSellersHeader = '<div class="editorial-vitrine-grid">';
  const bestSellersStart = content.indexOf('id="mas-vendidos"');
  if (bestSellersStart === -1) {
    console.error('Could not find id="mas-vendidos" in ' + filePath);
    return;
  }
  const bsGridStart = content.indexOf(bestSellersHeader, bestSellersStart);
  const bsSectionEnd = content.indexOf('</section>', bsGridStart);
  const bsGridEnd = content.lastIndexOf('</div>', bsSectionEnd);

  const existingBestSellersDemo = content.slice(bsGridStart + bestSellersHeader.length, bsGridEnd);

  const dynamicBestSellers = `
      ...
      {%- if bs_collection != blank and bs_collection.products.size > 0 -%}
        {%- for product in bs_collection.products limit: 4 -%}
          ...
          <div class="product-shop-card cutout-product-card" 
            ...
            <span class="badge-gold-bestseller">✦ Top #{{ forloop.index }} Mas Vendido</span>
            ...
            <div class="cutout-card-content">
              <div class="cutout-title-row">
                ...
                <span class="spec-weight-tag">{{ karat_tag | upcase }} Oro Autentico · Envio Asegurado</span>
              </div>
        ...
        {%- endfor -%}
      {%- else -%}
        ${existingBestSellersDemo.trim()}
      {%- endif -%}
    `;

  content = content.slice(0, bsGridStart + bestSellersHeader.length) + dynamicBestSellers + '\n    ' + content.slice(bsGridEnd);

  // 2. NEW ARRIVALS VITRINE REPLACEMENT
  const newArrivalsStart = content.indexOf('id="nuevas-colecciones"');
  if (newArrivalsStart === -1) {
    console.error('Could not find id="nuevas-colecciones" in ' + filePath);
    return;
  }
  const naGridStart = content.indexOf(bestSellersHeader, newArrivalsStart);
  const naSectionEnd = content.indexOf('</section>', naGridStart);
  const naGridEnd = content.lastIndexOf('</div>', naSectionEnd);

  const existingNewArrivalsDemo = content.slice(naGridStart + bestSellersHeader.length, naGridEnd);
  ...
  const dynamicNewArrivals = `
      ...
      {%- elsif new_collection != blank and new_collection.products.size > 0 -%}
        ...
        {%- endfor -%}
      {%- else -%}
        ${existingNewArrivalsDemo.trim()}
      {%- endif -%}
    `;

  content = content.slice(0, naGridStart + bestSellersHeader.length) + dynamicNewArrivals + '\n    ' + content.slice(naGridEnd);

  // 3. MAIN CATALOG GRID REPLACEMENT
  // Search for: <div class="catalog-products-grid" id="catalogProductsGrid"> ... </div>
  const catalogGridHeader = '<div class="catalog-products-grid" id="catalogProductsGrid">';
  const catGridStart = content.indexOf(catalogGridHeader);
  if (catGridStart === -1) {
    console.error('Could not find catalogProductsGrid in ' + filePath);
    return;
  }
  const catPaginationStart = content.indexOf('<!-- Dynamic Pagination Controls', catGridStart);
  const catGridEnd = content.lastIndexOf('</div>', catPaginationStart);

  // Clean up any existing if/endif wrappers around catalog grid
  let beforeCatGrid = content.slice(0, catGridStart);
  // Remove trailing assign/if statements before catalog grid if present
  const assignCheck = beforeCatGrid.lastIndexOf('{%- assign store_collection');
  if (assignCheck !== -1 && assignCheck > beforeCatGrid.lastIndexOf('<!-- Products Grid -->')) {
    beforeCatGrid = beforeCatGrid.slice(0, beforeCatGrid.lastIndexOf('<!-- Products Grid -->')) + '<!-- Products Grid -->\n      ';
  }

  // Extract inner demo cards
  let existingCatalogDemo = content.slice(catGridStart + catalogGridHeader.length, catGridEnd);
  // Remove any trailing {%- endif -%}
  existingCatalogDemo = existingCatalogDemo.replace(/\{%-\s*endif\s*-%\}$/s, '').trim();

  const dynamicCatalog = `{%- assign catalog_collection = section.settings.collection | default: collections['all'] -%}
      <div class="catalog-products-grid" id="catalogProductsGrid">
        {%- if catalog_collection != blank and catalog_collection.products.size > 0 -%}
          ...
          {%- endfor -%}
        {%- else -%}
          ${existingCatalogDemo.trim()}
        {%- endif -%}
      </div>`;

  let afterCatGrid = content.slice(catGridEnd + 6); // skip </div>
  // If afterCatGrid starts with {%- endif -%}, remove it
  afterCatGrid = afterCatGrid.replace(/^\s*\{%-\s*endif\s*-%\}[\r\n]*/, '');

  content = beforeCatGrid + dynamicCatalog + '\n\n      ' + afterCatGrid.trimStart();

  fs.writeFileSync(filePath, content, 'utf8');
  console.log('Successfully updated dynamic Shopify product loops in: ' + filePath);
}

updateStorefrontFile('sections/bonita-luxury-storefront.liquid');
if (fs.existsSync('code/sections/bonita-luxury-storefront.liquid')) {
  updateStorefrontFile('code/sections/bonita-luxury-storefront.liquid');
}
