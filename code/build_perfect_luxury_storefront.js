...
// Replace mock cameras in fallback demo cards with real asset images
...
let imgIndex = 0;
body = body.replace(/<div class="mock-camera-placeholder">[\s\S]*?<\/div>\s*<\/div>/g, () => {
            ...
            <div class="product-shop-card cutout-product-card" data-category="{{ cat_tag }}" data-karat="{{ karat_tag }}" data-price="{{ product.price | divided_by: 100 }}" data-weight="15.0" data-size="20">
              ...
              <div class="cutout-card-content">
                ...
                <div class="product-card-pricing-row">
                  ...
                  <button class="btn-add-cart-pill" onclick="window.location.href='{{ product.url }}'" title="Ver y Comprar" aria-label="Ver y Comprar">
...
const catalogGridTarget = '<div class="catalog-products-grid" id="catalogProductsGrid">';
const catalogGridIndex = body.indexOf(catalogGridTarget);

if (catalogGridIndex !== -1) {
  body = body.slice(0, catalogGridIndex) + dynamicShopifyProductLoop + body.slice(catalogGridIndex);
}

// Close the if-else block right before the trust pillars section
const trustPillarsTarget = '<section class="trust-pillars-section"';
const trustPillarsIndex = body.indexOf(trustPillarsTarget);

if (trustPillarsIndex !== -1) {
  body = body.slice(0, trustPillarsIndex) + '{%- endif -%}\n      </div>\n    </div>\n  </section>\n\n  <!-- TRUST PILLARS -->\n  ' + body.slice(trustPillarsIndex);
}
...
console.log('PERFECT PIXEL-EXACT LUXURY STOREFRONT REBUILT (CARDS, SVGS, FOOTER, SHOPIFY CATALOG)!');
